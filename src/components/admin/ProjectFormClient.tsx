    ...
    fetch(`/api/admin/projects/${projectId}`)
      ...
      .then((p: Project) => {
        setForm({
          ...
          scopeOfWork:    p.scopeOfWork?.length ? p.scopeOfWork : [""],
          toolsAndTech:   p.toolsAndTech?.length ? p.toolsAndTech : [""],
          challenge:      p.challenge ?? "",
          solution:       p.solution ?? "",
          results:        p.results?.length ? p.results : [""],
          valueDelivered: p.valueDelivered?.length ? p.valueDelivered : [""],
          media:          p.media ?? [],
          contentBlocks:  withMandatoryBlocks(p.contentBlocks ?? []),
