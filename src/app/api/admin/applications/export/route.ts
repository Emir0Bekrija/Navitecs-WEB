...
export async function POST(request: NextRequest) {
  ...
  const applicants = applicantIds.length > 0
    ? await prisma.applicant.findMany({ where: { id: { in: applicantIds } }, orderBy: { createdAt: "asc" } })
    : [];
  ...
  for (const [, { zipName, absolutePath }] of cvFiles) {
    try {
  ...
  if (mode === "stale") {
    ...
    const recentApplicantIds = await prisma.application
      .findMany({
        where: { submittedAt: { gte: cutoff }, applicantId: { in: applicantIds } },
        select: { applicantId: true },
        distinct: ["applicantId"],
      })
      .then((rows) => new Set(rows.map((r) => r.applicantId!)));
    ...
    if (applicantIdsToDelete.length > 0) {
      await prisma.applicant.deleteMany({ where: { id: { in: applicantIdsToDelete } } });
    }
