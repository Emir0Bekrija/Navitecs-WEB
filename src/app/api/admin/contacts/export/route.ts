...
export async function POST(request: NextRequest) {
  ...
  if (mode === "stale") {
    ...
    const recentCompanyContactIds = await prisma.contact
      .findMany({
        where: { submittedAt: { gte: cutoff }, companyContactId: { in: companyContactIds } },
        select: { companyContactId: true },
        distinct: ["companyContactId"],
      })
      .then((rows) => new Set(rows.map((r) => r.companyContactId!)));
    ...
    if (companyContactIdsToDelete.length > 0) {
      await prisma.companyContact.deleteMany({ where: { id: { in: companyContactIdsToDelete } } });
    }
  }
