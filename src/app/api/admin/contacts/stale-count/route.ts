...
export async function GET() {
  ...
  const [staleContacts, recentCompanyContactIds] = await Promise.all([
    ...
    prisma.contact.findMany({
      where: { submittedAt: { gte: cutoff }, companyContactId: { not: null } },
      select: { companyContactId: true },
      distinct: ["companyContactId"],
    }),
  ]);

  const recentIds = new Set(recentCompanyContactIds.map((r) => r.companyContactId!));

  // Company contacts with only stale submissions
  const staleCompanyContacts = await prisma.companyContact.count({
    where: {
      id: { notIn: [...recentIds] },
      contacts: { some: { submittedAt: { lt: cutoff } } },
    },
  });
